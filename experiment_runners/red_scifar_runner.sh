#!/bin/bash
# filepath: /home/emily/makram/compressm/experiment_runners/red_scifar_runner.sh

# Define the path to the configuration file
CONFIG_FILE="experiment_configs/repeats/lru/scifar.json"

# Define the different ssm_dim values to test (larger dimensions for reduction)
SSM_DIMS=(384)
...
run_experiment_group() {
    ...
    local group_window_name="red_scifar_gpu${gpu_id}_group${group_num}"
    
    echo "=========================================="
    echo "Starting REDUCED SCIFAR experiment group $group_num on GPU $gpu_id"
    ...
    local cmd="cd $(pwd) && echo 'Starting REDUCED SCIFAR experiment group $group_num on GPU $gpu_id'"
    ...
    echo "REDUCED SCIFAR experiment group $group_num launched on GPU $gpu_id"
}

# Main execution
echo "Starting REDUCED SCIFAR SSM dimension experiments"

# Check if tmux session exists, if not create one
if ! tmux has-session -t red_scifar_experiments 2>/dev/null; then
    echo "Creating new tmux session: red_scifar_experiments"
    tmux new-session -d -s red_scifar_experiments -c $(pwd)
else
    echo "Using existing tmux session: red_scifar_experiments"
fi
...
echo "=== PHASE 1: Running REDUCED SCIFAR grid search experiments in groups (sequential per GPU) ==="
...
echo ""
echo "All REDUCED SCIFAR experiment groups launched!"
echo "Total experiments: $total_experiments distributed across ${#GPU_ASSIGNMENTS[@]} GPUs"
echo "To monitor experiments, use: tmux attach -t red_scifar_experiments"
echo "To list all windows: tmux list-windows -t red_scifar_experiments"
echo ""
echo "REDUCED SCIFAR experiment summary:"
...
echo "1. Attach to tmux session:    tmux attach -t red_scifar_experiments"
echo "2. List all windows:          tmux list-windows -t red_scifar_experiments"
echo "3. Switch to GPU group N:     tmux select-window -t red_scifar_experiments:red_scifar_gpu<N>_group<N>"
echo "4. Kill all experiments:      tmux kill-session -t red_scifar_experiments"
echo "5. Monitor GPU usage:         watch -n 2 nvidia-smi"
echo ""
echo "Window naming: red_scifar_gpu<GPU_ID>_group<GROUP_NUM>"
echo ""
echo "=== REDUCED SCIFAR EXPERIMENT GRID DETAILS ==="
